...
public class T14ThreadShareDataTest {
	 ...
	 * 스레드에서 데이터를 공유하는 방법
	 * 
	 * 1.공유데이터를 관리하기 위한 클래스 정의한다. 2.클래스를 이용하여 공유객체를 생성한다. 3.이 인스턴스를 각각의 스레드에 제공한다.
	 * 4.각각의 스레드는 이 인스턴스에 저장한 데이터를 공유하여 사용한다.
	 * 
	 * 예) 원주율을 계산하는 스레드가 있고, 계산된 원주율을 출력하는 스레드가 있다. 원주율을 계산한 후 이 값을 출력하는 프로그램 작성하기.
	 */
	public static void main(String[] args) {
		ShareData sd = new ShareData();
		
		CalcPIThread cTh = new CalcPIThread(sd);//공유객체
...
class ShareData {
	...
	volatile private boolean isOk; // 변덕스러운(volatile)+원주율 계산 완료여부 확인용 변수
	/*
	 volatile => 선언된 변수를 컴파일러에 최적화대상에서 제외시킨다.
